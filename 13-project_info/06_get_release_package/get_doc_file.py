...
def delete_special_files(backup_dir, table_job_list):
    logger.info('----------------- 删除特殊文件 -------------')
    special_dir = os.path.join(os.path.dirname(backup_dir), 'special_tables')
    ...
    for idx, (table_name, job_number, job_person, special_flag) in enumerate(table_job_list):
        if special_flag == 'Y':
            ...
            if len(create_table_files) == 1:
                ...
                shutil.copy2(create_table_file, special_table_file)  # 备份到 special_tables
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        logger.error("Usage: python get_doc_files.py &lt;上线包路径&gt; &lt;上线清单&gt;")
        sys.exit(1)
    base_dir = sys.argv[1]
