 ...
 'v2.0 2024/1/26  修复bug，添加功能
 'v2.1 2024/1/29  添加可视化信息，进度条和选择按钮 修复日期判断语句
 'v2.2 2024/1/30  重写优化删除重复数据的逻辑，修复工作日期顺序填写颠倒不提示的错误
...
End Sub
 ...
 Private Sub worksheetsgenerate()
    ...
    Const SOURCE_SHEET_NAME As String = "数据来源"
    ...
    Do While i <= lastRow
        ...
        empname = sourcews.Range("E" & startRow).Value
        ...
        '计算进度条百分比并增加相应宽度
        With urfProgress
            .lblCaption.Caption = "正在处理" & empname & "的个人简历."
            .lblProgress.Width = pct * (.fraProgress.Width)
        End With
        DoEvents
        ...
        If Trim(sourcews.Range("K" & startRow).Value) <> "" Then
            ...
            Else
                education_date = Format(sourcews.Range("H" & startRow).Value, "yyyy年mm月")  '毕业时间(单一学历，但填了两边)
                education_school = sourcews.Range("I" & startRow).Value    '毕业学校
                education_major = sourcews.Range("J" & startRow).Value    '专业
            End If
        Else
            education_date = Format(sourcews.Range("H" & startRow).Value, "yyyy年mm月")  '毕业时间(单一学历)
            education_school = sourcews.Range("I" & startRow).Value  '毕业学校
            education_major = sourcews.Range("J" & startRow).Value   '专业
        End If
        ...
        deptname = Trim(RemoveEnglishCharacters(sourcews.Range("B" & startRow).Value))
        ...
        Call CreateVisibleWordAppAndSaveAsDocx(empname, deptname, folderpath)
    ...
    'infolderpath = folderpath & "\" & deptname
    'Call resume_beauty
...
Private Sub ValidateAndDisplayData()
    ...
    Dim first_bac_date As Variant  '第一毕业时间
    Dim first_bac_school As Variant '第一毕业学校
    Dim first_bac_major As Variant '第一毕业专业
    
    Dim last_bac_date As Variant  '最高毕业时间
    Dim last_bac_school As Variant '最高毕业学校
    Dim last_bac_major As Variant '最高毕业专业
    
    Dim second_bac_date As Variant  '第二毕业时间
    Dim second_bac_school As Variant '第二毕业学校
    Dim second_bac_major As Variant '第二毕业专业

    
    Dim third_bac_date As Variant  '第三毕业时间
    Dim third_bac_school As Variant '第三毕业学校
    Dim third_bac_major As Variant '第三毕业专业
        
        '赋值
    
    
    ' 选择你的工作表
    Set sourcews = ThisWorkbook.Worksheets("数据来源") ' 将"Sheet1"替换为你的工作表名称
    ...
    Do While i <= lastRow
        ...
        empname = sourcews.Range("E" & startRow).Text ' 姓名
        ...
        'graduationDate = sourcews.Range("H" & startRow).Value ' 毕业时间
        ...
        Else
            validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的开始工作日期不是日期格式; " + Chr(13)
        End If
       
        
        '/////校验学历信息是否填写正确（漏填错填）/////
        '漏填的情况 主要是检测 最高学历 第二学历 第一学历是否填写
     
        first_bac_date = sourcews.Range("K" & startRow).Value
        first_bac_school = sourcews.Range("L" & startRow).Text
        first_bac_major = sourcews.Range("M" & startRow).Text

        last_bac_date = sourcews.Range("H" & startRow).Value
        last_bac_school = sourcews.Range("I" & startRow).Text
        last_bac_major = sourcews.Range("J" & startRow).Text
        
        second_bac_date = sourcews.Range("AY" & startRow).Value
        second_bac_school = sourcews.Range("AZ" & startRow).Text
        second_bac_major = sourcews.Range("BA" & startRow).Text
        
        third_bac_date = sourcews.Range("BB" & startRow).Value
        third_bac_school = sourcews.Range("BC" & startRow).Text
        third_bac_major = sourcews.Range("BD" & startRow).Text
        
        '检测第一学历是否填写

        
        If last_bac_date = "" Or last_bac_school = "" Or last_bac_major = "" Then
            validationResultDescription = "【重要】第 " & i & " 行数据中，" & empname & "的最高学历信息未填写或未填写完整; " + Chr(13)

        '检测最高学历是否未填写 填写是否完毕的内容

        End If
        
        If (first_bac_date <> "" And (first_bac_school = "" Or first_bac_major = "")) Or (first_bac_school <> "" And (first_bac_date = "" Or first_bac_major = "")) Or (first_bac_major <> "" And (first_bac_school = "" Or first_bac_date = "")) Then
    
            validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的第一学历信息未填写完整; " + Chr(13)
            
        End If
    
        If (second_bac_date <> "" And (second_bac_school = "" Or second_bac_major = "")) Or (second_bac_school <> "" And (second_bac_date = "" Or second_bac_major = "")) Or (second_bac_major <> "" And (second_bac_school = "" Or second_bac_date = "")) Then
    
            validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的第二学历信息未填写完整; " + Chr(13)
            
        End If
    
        If (third_bac_date <> "" And (third_bac_school = "" Or third_bac_major = "")) Or (third_bac_school <> "" And (third_bac_date = "" Or third_bac_major = "")) Or (third_bac_major <> "" And (third_bac_school = "" Or third_bac_date = "")) Then
    
            validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的第三学历信息未填写完整; " + Chr(13)
            
        End If
        
        
        '获取最早的毕业日期按已经填写的来算
        graduationDate = last_bac_date
        
        ' 比较并更新graduationDate
        If Not IsEmpty(first_bac_date) And first_bac_date < graduationDate Then
            graduationDate = first_bac_date
        End If
        
        If Not IsEmpty(second_bac_date) And second_bac_date < graduationDate Then
            graduationDate = second_bac_date
        End If
        
        If Not IsEmpty(third_bac_date) And third_bac_date < graduationDate Then
            graduationDate = third_bac_date
        End If
        ...
        If IsDate(graduationDate) And IsDate(startDate) Then
            ...
            If graduationDate > startDate Then
                ' 显示错误信息（可以根据需要进行处理）
                validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的毕业日期晚于开始工作日期，请检查数据是否填写正确; " + Chr(13)
            End If
            
            ' 检查工作年限是否填写
            If yearsOfWork = "" Then
                ' 存储校验结果到字典
                validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的工作年限未填写; " + Chr(13)
            Else
                ' 检查工作年限是否合理 掐头去尾
                If InStr(yearsOfWork, "年") > 1 Then
                
                    validationResultDescription = validationResultDescription & "【一般】第 " & i & " 行数据中，" & empname & "的工作年限填写了“年”字段，请检查（应为" & yearsOfWork_m & "）; " + Chr(13)
                    yearsOfWork = Replace(yearsOfWork, "年", "")
                    
                    If Abs(yearsOfWork_m - yearsOfWork) >= 1 Then
                    ' 存储校验结果到字典
                        validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的工作年限与实际经验差距大，请检查（应为" & yearsOfWork_m & "）; " + Chr(13)
                    End If
                End If
            End If
        Else
            ' 如果日期无效，存储校验结果到字典
            validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的毕业日期或开始工作日期无效; " + Chr(13)
        End If
        ...
        If IsDate(startprojectdate) And IsDate(startDate) Then
            ...
            If startprojectdate < startDate Then
                ' 显示错误信息（可以根据需要进行处理）
                validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的初次项目日期早于开始工作日期，请检查数据是否填写正确; " + Chr(13)
        
            End If
        
        Else
            ' 如果日期无效，存储校验结果到字典
                validationResultDescription = validationResultDescription & "【重要】第 " & i & " 行数据中，" & empname & "的初次项目日期或开始工作日期无效; " + Chr(13)
        End If
        ...
        Dim dateQ1 As Variant
        Dim dateR1 As Variant
        
        ' 存储单元格的值，以提高效率
        dateQ1 = sourcews.Range("Q" & j).Value
        dateR1 = sourcews.Range("R" & j).Value
        
        
        If IsDate(dateQ1) And IsDate(dateR1) Then
            ' 值是日期格式，进行相应的操作
            Dim validDateQ As Date
            Dim validDateR As Date
            
            ' 将值转换为 Date 类型
            validDateQ = CDate(dateQ1)
            validDateR = CDate(dateR1)
            If validDateQ > validDateR Then
            
                validationResultDescription = validationResultDescription & "【重要】第 " & j & " 行数据中，" & empname & "的工作日期顺序填写有误，请检查; " + Chr(13)
        ...
        Else
           ...
           If dateQ1 = "至今" Then
              ...
              validationResultDescription = validationResultDescription & "【重要】第 " & j & " 行数据中，" & empname & "的工作日期开始不能填写为“至今”，请检查; " + Chr(13)
        ...
        validationResultDescription = validationResultDescription & CheckDateRanges(startRow, endRow) & Chr(13)
            ...
            For tt = startRow To maxnoemptyrow
                If maxnoemptyrow - startRow + 1 <= 10 Or maxnoemptyrow - startRow = 0 Then
                    ...
                    If IsBankOrCompanyProject(sourcews.Range(colLabelAsString & tt)) = False Then
                        ...
                        validationResultDescription = validationResultDescription & "【一般】第 " & colLabelAsString & " 列 " & tt & " 行数据中，“" & projectname & "”中项目名称没有按照{XX公司/联社/银行 XX项目}格式填写; " + Chr(10)
                    ...
                    If regex.test(adjacentCellValue1) And regex.test(adjacentCellValue2) Then
                        ...
                        If adjacentCellValue1 > adjacentCellValue2 Then
                            ...
                            validationResultDescription = validationResultDescription & "【一般】第 " & tt & " 行数据中，“" & projectname & "” 项目的开始日期与结束日期的顺序填写错误; " + Chr(13)
                    ...
                    Else
                        ...
                        validationResultDescription = validationResultDescription & "【一般】第 " & colLabelAsString & " 列 " & tt & " 行数据中，“" & projectname & "”中日期时间没有按照yyyy/mm格式填写; " + Chr(13)
        ...
        If Replace(Trim(validationResultDescription), Chr(13), "") <> "" Then
        
            validationResults(i) = Trim(validationResultDescription)
    ...
    For Each Key In validationResults.Keys
        ' 检查对应的值是否为空
        If Trim(validationResults(Key)) = "" Then
            ' 如果值为空，从字典中删除该键
   ...
   ' 选择 A1 单元格及其当前区域
    ...
    newSheet.Columns("C:C").ColumnWidth = 180
...
Function GetMaxRowInColumnAWithMerge(ws As Worksheet) As Long
    ...
    Dim cell As Range
    'Dim ws As Worksheet
    'Set ws = ThisWorkbook.Worksheets("数据来源") ' 替换为你的工作表名称
...
Private Sub CreateVisibleWordAppAndSaveAsDocx(ByVal empname As String, ByVal deptname As String, ByVal folderpath As String)
    ' 创建一个可见的 Word 应用程序实例
    ...
    Dim docName As String
    docName = folderpath & "\" & deptname & "\个人简历_" & emplyname & "_" & Format(Date, "yyyymmdd") & ".docx"
    
    ' 保存当前处理人员的表格到新的 docx 文件中
    
    '查看对应部门文件夹是否存在
    
    If Dir(folderpath & "\" & deptname, vbDirectory) = "" Then
        On Error Resume Next
        'Kill folderpath & "\" & deptname & "\*.*"

        MkDir folderpath & "\" & deptname
        On Error GoTo 0
    Else
                ' 检测文件是否存在
        If Dir(docName) <> "" Then
    ...
    If Not maxSheet Is Nothing Then
        ...
        ' 处理文档中的所有表格
        For Each resumeTable In wordDoc.Tables
            ' 设置表格为根据窗口自自动调整
...
Private Sub CleanYearFromCell()
    ...
    ' 选择你的工作表
    Set ws = ThisWorkbook.Worksheets("数据来源") ' 将"Sheet1"替换为你的工作表名称
...
Private Sub DeleteOtherSheets()
    ...
    For Each ws In ThisWorkbook.Sheets
        If ws.Name <> "数据来源" And ws.Name <> "Template" Then
            ws.Delete
        End If
    Next ws
...
Private Sub get_project_info(ByVal startRow As Long, ByVal endRow As Long)
    ...
    ' 选择你的工作表
    Set ws = ThisWorkbook.Worksheets("数据来源") ' 将"Sheet2"替换为你的工作表名称
...
Function FindEarliestDate(ws As Worksheet, QStartRow As Long, QEndRow As Long, Optional ATStartRow As Variant, Optional ATEndRow As Variant) As String
    ...
    ' 设置日期范围AT列，如果有传入参数
    If Not IsMissing(ATStartRow) And Not IsMissing(ATEndRow) Then
...
Private Function CheckDateRanges(ByVal startRow As Long, ByVal endRow As Long)
    ...
    Set mainWS = ThisWorkbook.Sheets("数据来源") ' 请替换为你的主工作表的名称
    ...
    ' 循环主工作表的 Q 列和 R 列数据
    For i = startRow To lastQ
        Dim dateQ As Variant
        Dim dateR As Variant
        
        dateQ = mainWS.Cells(i, "Q").Value
        dateR = mainWS.Cells(i, "R").Value
        
        If IsDate(dateQ) And IsDate(dateR) Then
              dateQ = mainWS.Cells(i, "Q").Value
              dateR = mainWS.Cells(i, "R").Value
              
            ElseIf Not IsDate(dateQ) Then
            

                errorMessage = errorMessage & "【重要】工作经历日期格式填写错误：工作经历第 " & i & " 行 Q列,填写内容为 { " & mainWS.Cells(i, "Q").Value & "};" + Chr(13)
            
            
            ElseIf Not IsDate(dateR) Then
            
                If dateR = "至今" Then
                    dateR = Format(Now(), "yyyy/mm")
                Else

                    errorMessage = errorMessage & "【重要】工作经历日期格式填写错误：工作经历第 " & i & " 行 R列,填写内容为 { " & mainWS.Cells(i, "R").Value & "};" + Chr(13)
        ...
        For j = 1 To lastMainRow
            ' 从数据工作表获取日期范围

            '判断日期填写是否合规,看能否转化为日期格式 是否错误
            ...
            ElseIf Not IsDate(dateA) Then
            
            errorMessage = errorMessage & "【重要】项目日期格式填写错误：工作经历第 " & j & " 行 A 列;" + Chr(13)
            GoTo NEXT_DATE
            
            
            ElseIf Not IsDate(dateB) Then
            
                If dateB = "至今" Then
                    dateB = Format(Now(), "yyyy/mm")
                Else
                
                   errorMessage = errorMessage & "【重要】项目日期格式填写错误：工作经历第 " & j & " 行 B 列;" + Chr(13)
                    GoTo NEXT_DATE
                End If
            ...
            If (dateA < dateQ And dateQ < dateB) Or (dateA < dateR And dateR < dateB) Then
                errorMessage = errorMessage & "【重要】跨工作经历的项目日期错误：工作经历第 " & i & " 行Q列，工作经历：" & mainWS.Cells(i, "S").Value & ",项目名称：" & dataWS.Cells(j, "C").Value & vbCrLf
            End If
    ...
    If lastAT > 1 Then
        ...
        For k = startRow To lastAT ' 假定第一行是标题行，从第二行开始
            ...
            For l = 1 To lastMainRow ' 假定第一行是标题行，从第二行开始
                ...
                ' 检查日期范围是否存在错误
                If (dateA < dateQ And dateQ < dateB) Or (dateA < dateR And dateR < dateB) Then
                    errorMessage = errorMessage & "【重要】跨工作经历的项目日期错误：工作经历第 " & k & " 行AT列，工作经历：" & mainWS.Cells(k, "AV").Value & ",项目名称：" & dataWS.Cells(l, "C").Value & vbCrLf
                End If
            Next l
NEXT_LINE:
            errorMessage = errorMessage & "【重要】项目日期格式填写错误：工作经历第 " & k & " 行 AT 列附近;" + Chr(13)
