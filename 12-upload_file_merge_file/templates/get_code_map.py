...
def process_code_mapping(table_list_file): 
    ...
    error_log=f"error_log{person_name}.txt" 
    ...
    for table_name in table_names: 
        print(f"正在处理<{person_name}>-<{table name}>的码值映射。") 
        code_map_files=[ 
            os.path.join('pub_cd map',f'pub_cd_map-(person_name).xlsx'), 
        ...
        if not code_map_file: 
            1og_error(error_log,f"{person_name}的代码映射文件不存在.") 
            sys.exit() 
        try: 
            ...
            if tgt_cm_data: 
                ...
                tgt cm sheet.clear_contents()
                tgt_cm sheet.range('A1').value = [tgt_cn_data[0]] + tgt_cm_df.values.tolist()
            #读取并筛选源文件中的码值映射数据
            ...
            if src_cm_data: 
                ...
                if filtered_src_cm_df.empty: 
                    log_error(error_log,f"{code_map_file}中未找到{table name}表的代码映射.")
                else:
