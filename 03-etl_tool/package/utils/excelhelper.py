...
class ExcelRW:
    ...
    def __init__(self, path):
        if license.verify():
            ...
            _logger.error("license check failed!")
            _logger.error("prosses failed !!!")
            sys.exit(-1)
    ...
    def get_excel_type(self):
        ...
        # _logger.info("excel_type: " + excel_type)
        return excel_type
    ...
    def get_data(self):
        if self.type == "2003":
            ...
            _logger.error("unknow excel type, exit!, file name: %s" % self.path)
...
class Xls:
    def __init__(self, path):
        ...
        self.data = Xls.get_book_data(self.book)
...
class Xlsx:
    ...
    def set_data(self, date, skip_line):
        return True


if __name__ == '__main__':
    file_path = "/etlscript/data/excel/test.xlsx"
    excel = ExcelRW(file_path)
    excel.get_data()
    print(excel.data.keys())
    print(excel.data["Sheet2"][5])
    print(excel.data["Sheet2"][5][4])
