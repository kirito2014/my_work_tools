...
def process_folder(folder_path):
    data = []  # 存储文件名、belong_theme 和 表名 的列表
        ...
        if file_name.endswith(".sql"):
            ...
            with open(file_path, 'r', encoding='gbk') as file:
                ...
                for table_name in filtered_table_names:
                    data.append((file_name, belong_theme, table_name))
    return data
...
def write_to_excel(data, output_file):
    ...
    sheet['C1'] = '表名'
    sheet['D1'] = 'query_target'
    sheet['E1'] = 'query_source'

    # 写入数据
    for idx, (file_name, belong_theme, table_name) in enumerate(data, start=2):
        ...
        # 在 D2 单元格填写公式
        if idx == 2:
            formula = '="INSERT INTO AGL_COMM.{} SELECT ''TT'' ,'' ''&UPPER(TRIM(B{}))&'' '','''',COUNT(1),''2023-09-30'' FROM ''&A{}&''.''&B{}&'' WHERE PT_DT = ''2023-09-30'';"'.format(table_name, idx, idx, idx)
            sheet.cell(row=idx, column=4).value = formula
        else:
            formula = '="INSERT INTO AGL_COMM.{} SELECT ''TT'' ,'' ''&UPPER(TRIM(B{}))&'' '','''',COUNT(1),''2023-09-30'' FROM ''&A{}&''.''&B{}&'' WHERE PT_DT = ''2023-09-30'';"'.format(table_name, idx, idx, idx)
            # 替换单引号为两个单引号
        ...
        # 在 E 列写入固定值
        sheet.cell(row=idx, column=5, value="Your query source")
